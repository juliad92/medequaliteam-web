...
export default function VolunteerImportantNotice({ locale }: { locale: string }) {
  ...
  return (
    <div role="note" className="mb-8 rounded-2xl border border-amber-200/80 bg-amber-50 px-6 py-5">
      ...
      <ul className="mt-3 space-y-2">
        <li className="flex gap-3 text-[15px] leading-relaxed text-amber-950/85">
          <span
            className="mt-1.5 h-1.5 w-1.5 flex-shrink-0 rounded-full bg-amber-600"
            aria-hidden="true"
          />
          <span>{copy.visa}</span>
        </li>
