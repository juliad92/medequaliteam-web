...
async function ensureProject(payload: Payload, slug: string, title: string) {
  const { docs } = await payload.find({
    collection: 'projects',
...
async function ensurePublishedVolunteerNeed(payload: Payload, projectId: string) {
  const { docs } = await payload.find({
    collection: 'volunteer-needs',
    where: {
      and: [{ project: { equals: projectId } }, { roleName: { equals: E2E.roleName } }],
    },
    limit: 1,
    overrideAccess: true,
  })
...
export async function seedVolunteerE2EFixtures(): Promise<SeedState> {
  return withPayload(async (payload) => {
    ...
    const project = await ensureProject(payload, E2E.projectSlug, 'E2E Volunteer Project')
